...
RUN apt update && apt install -y git nano python3 python3-pip libssl-dev && \
...
RUN cd $ROOT_DIR && \
    ...
    git checkout to_async && \
    virtualenv $ROOT_DIR/env && \
    source $ROOT_DIR/env/bin/activate && \
    cd src && pip3 install -r requirements.txt
    # pip3 install quart pycryptodome base58 bson fastjsonschema fasttext tqdm sentence_transformers chardet
